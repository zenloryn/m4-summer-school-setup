...
if ! uv add "m4-infra @ git+https://github.com/CMI-Laboratory/m4.git"; then
...
if [ ! -f "$CVD_JSON_PATH" ]; then
    ...
    if ! curl -fsSL https://raw.githubusercontent.com/CMI-Laboratory/m4-summer-school-setup/main/cvd.json -o "$CVD_JSON_PATH"; then
